...
export const ruMessages: AppMessages = {
  ...
  flow: {
    ...
    metamapDemoHint:
      "Только для теста без документа: симулируйте следующий шаг. В проде кнопка выше открывает реальную проверку (документ и селфи) по сниппету из dashboard MetaMap.",
    metamapDemoButton: "Продолжить без проверки (демо)",
    metamapSkipHint:
      "Кнопка выше открывает проверку с **документом и селфи** (MetaMap создаёт сессию по нажатию; **MATI_CLIENT_SECRET** для пробы не нужен). Для **интеграции или QA** можно пропустить и продолжить с демо-данными.",
    ...
    metamapProvisionFailed:
      "Не удалось создать identity через API (опционально). Для реального селфи используйте кнопку проверки; достаточно **clientId** и **flowId** из dashboard.",
    metamapProvisionRetry: "Повторить",
    ...
    doneReviewH1: "Проверка на рассмотрении",
    doneReviewBody:
      "Мы получили документ и селфи. Команда должна **вручную проверить** случай до завершения смены профиля. Мы сообщим, когда процесс продолжится; повторно отправлять проверку сейчас не нужно.",
    doneRejectedH1: "Личность не подтверждена",
    doneRejectedBody:
      "Проверка личности **не одобрена**. Убедитесь, что документ читаем и действителен, попробуйте позже или обратитесь в поддержку Punto Pago.",
  ...
  migration: {
    ...
    statusProcessing: "Проверяем результат вашей верификации…",
    statusVerified: "Личность подтверждена. Завершаем регистрацию переноса…",
    statusReviewNeeded:
      "Верификация **ожидает ручной проверки**. Продолжим после одобрения командой.",
    statusRejected: "Верификация не одобрена. Готовим итог…",
  },
  metamapUi: {
