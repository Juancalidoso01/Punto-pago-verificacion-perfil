...
export const ruMessages: AppMessages = {
  ...
  flow: {
    ...
    metamapDemoHint:
      "Без валидного **identityId** SDK Mati не откроется. Кнопка ниже имитирует проверку и позволяет пройти остальной поток; в продакшене нужен `?identityId=…` или `NEXT_PUBLIC_METAMAP_IDENTITY_ID` для тестов.",
